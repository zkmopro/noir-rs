...
/// Returns an error if any string cannot be parsed as a field element.
pub fn from_vec_str_to_witness_map(
    ...
    for (i, witness) in witness_vec.iter().enumerate() {
        let field = FieldElement::try_from_str(*witness)
            .ok_or_else(|| format!("Failed to parse witness at index {}: '{}'", i, witness))?;
        witness_map.insert(Witness(i as u32), field);
    }
