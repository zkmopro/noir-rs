...
/// * `low_memory_mode` - Whether to use file-backed memory (slower but uses less RAM)
///
/// # Returns
/// * The proof as a flat byte vector: [public_inputs_bytes][proof_bytes]
...
/// Generate an Ultra Honk proof with extended options.
///
/// Same as `prove_ultra_honk` but with optional storage budget control.
pub fn prove_ultra_honk_with_options(
    circuit_bytecode: &str,
    initial_witness: WitnessMap<FieldElement>,
    verification_key: Vec<u8>,
    low_memory_mode: bool,
    ...
    // The prefix lets verify_ultra_honk split inputs from proof fields.
...
pub fn prove_ultra_honk_keccak(
    ...
    prove_ultra_honk_keccak_with_options(
        circuit_bytecode,
        initial_witness,
        verification_key,
        disable_zk,
...
/// Generate an Ultra Honk Keccak proof with extended options.
///
/// Same as `prove_ultra_honk_keccak` but with optional storage budget control.
pub fn prove_ultra_honk_keccak_with_options(
    circuit_bytecode: &str,
    initial_witness: WitnessMap<FieldElement>,
    verification_key: Vec<u8>,
    disable_zk: bool,
